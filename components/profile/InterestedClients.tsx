...
const InterestedClients = ({enquiries}:any) => {
  const router = useRouter();
  const [open, setOpen] = useState(false);

  return (
    <div id="interested-clients">
        ...
        {({ open }) => (
          <>
            <Disclosure.Button className="w-full flex justify-between items-end bg-gray-100 p-2 mb-5">
              <p className="text-sm font-bold">Interested Clients</p>
              {/* <Disclosure.Button> */}
              {open ? (
                          ...
                          <tr key={enquiry?._id} className="bg-white">
                            ...
                            >
                              <div className="flex justify-center items-center">
                                <p className="mr-2">{enquiry?.property?.projectName}</p>
                                <span className="cursor-pointer">
                                  <GrShare onClick={()=> router.push(`/property/${enquiry?.property?._id}`)} />
                                </span>
                              </div>
                            </th>
                            ...
                            <td className="px-4 py-4 border-black border-[1px]  text-black">
                              {moment(new Date(enquiry?.createdAt)).format("YYYY-MM-DD h:mm a")}
                            </td>
                          </tr>
                        );
                      })}
